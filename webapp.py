...
def network_graph(yearRange, ticker_to_search):
    ...
    shells=[]
    shell1=[]
    shell1.append(ticker_to_search)
    shells.append(shell1)
    shell2=[]
    for symbol in ticker_set:
        if symbol!=ticker_to_search:
    ...
    if len(shell2)==0:
        ...
        node_trace = go.Scatter(x=tuple([1]), y=tuple([1]), text=tuple([str(ticker_to_search)]), textposition="bottom center",
                                mode='markers+text',
